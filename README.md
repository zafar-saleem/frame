<p>Frame is a JavaScript framework. It consists frame.js file that has core functionality. At the moment, frame API offers some useful and handy functions such as registering new modules, starting(initializing) and particular modules. It also offers aggregatedEvents system(inspired from backbonejs events system) and pub/sub. Frame is completely scalable framework which separates modules and ensure loose coupling among modules. framejs file also acts like an interface between two modules.</p>
<h1>How to use</h1>
